    ...
    constructor(props) {
      ...
      componentDidMount(){
        ...
        if(cookies.get('valid')!=1){
          ...
          fetch('http://localhost:3001/jwt?e='+ cookies.get('email') )
            ...
            .then(data1 => {
              fetch('http://d6b588abbd1f.ngrok.io/get?token='+ data1.token + "&date=" + this.state.selectedDate )
              .then(response => response.json())
              .then(data => {
                var ss  = [];
                data.Events.map((s)=>{
     ...
     handleChange(event) {   
      ...
      fetch('http://localhost:3001/jwt?e='+this.state.email )
            ...
            .then(data1 => {
              console.log(data1)
              fetch('http://d6b588abbd1f.ngrok.io/get?token='+ data1.token + "&date=" + event.getFullYear() + "-" + mes  + "-"  + dia )
              .then(response => response.json())
              .then(data => {
                var ss  = [];
                data.Events.map((s)=>{
